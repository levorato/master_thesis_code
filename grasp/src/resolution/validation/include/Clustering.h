...
#ifndef CLUSTERING_VALIDATION_H_
#define CLUSTERING_VALIDATION_H_
...
using namespace std;
using namespace problem::validation;
...
public:
	...
	/* Matrices used by RCC relaxed clusteringgraph::Imbalance calculation */
	 ...
	 */
	Clustering(ClusterArray &cArray, SignedGraph& g, clusteringgraph::validation::ClusteringProblem &p);
	...
	Clustering(ClusterArray &cArray, SignedGraph& g, clusteringgraph::validation::ClusteringProblem &p,
			...
			std::vector<clusteringgraph::Imbalance>& inProcessImbalance);
	 ...
	 */
	void addCluster(SignedGraph& g, clusteringgraph::validation::ClusteringProblem& p,
	 ...
	 * modification.
	 */
	void addNodeToCluster(SignedGraph& g, clusteringgraph::validation::ClusteringProblem& p,
	 ...
	 * modification.
	 */
	void removeNodeFromCluster(SignedGraph& g, clusteringgraph::validation::ClusteringProblem& p,
			const unsigned long& i, const unsigned long& k, bool updateImbalance = true);
	...
	clusteringgraph::Imbalance getObjectiveFunctionValue() const;

	void setObjectiveFunctionValue(clusteringgraph::Imbalance f);
	 ...
	 */
	const clusteringgraph::Imbalance& getImbalance() const {
		return imbalance;
	}

	void setImbalance(clusteringgraph::Imbalance imbalance) {
	...
	/**
	 * Returns a list containing the internal process clusteringgraph::Imbalance for each process in splitgraph.
	 */
	const std::vector<clusteringgraph::Imbalance>& getInternalProcessImbalance() const {
		return internalProcessImbalance;
	}

	/**
	 * Sets the internal clusteringgraph::Imbalance of a specific process.
	 */
	void setProcessImbalance(unsigned int processNumber, clusteringgraph::Imbalance imb) {
...
private:
	...
	clusteringgraph::Imbalance imbalance;
	...
	/** Stores the internal clusteringgraph::Imbalance (positive and negative) for a given process (splitgraph only) */
	std::vector<clusteringgraph::Imbalance> internalProcessImbalance;

...
namespace clusteringgraph {
	...
	__global__ void doubleSearchKernel(const float* weightArray, const int* destArray, const int* numArray,
		...
		int offset_i = offsetArray[i];
		int numedges_i = numArray[i];
		int offset_j = offsetArray[j];
		int numedges_j = numArray[j];
		// shared memory
		extern __shared__ long s[];       // n longs
		long *s_cluster = s;              // n longs
		for(ulong z = 0; z < n; z++) {
		...
		if(idx < numberOfChunks) {
			...
			// REMOVAL of vertex i from cluster k1 -> avoids recalculating 
			//   the same thing in the inner loop
			float negativeSumK1 = 0.0, positiveSumK1 = 0.0;
			// in/out-edges of vertex i
			ulong count = offset_i + numedges_i;
			for (ulong edgenum = offset_i; edgenum < count; edgenum++) {   
				int targ = destArray[edgenum];
				float weight = weightArray[edgenum];
				// REMOVAL from cluster k1: subtracts imbalance
				if(s_cluster[targ] == k1) {  // same cluster
			...
			// REMOVAL of vertex j from cluster k2 -> avoids recalculating 
			//   the same thing in the inner loop
			// in/out-edges of vertex j
			count = offset_j + numedges_j;
			for (ulong edgenum = offset_j; edgenum < count; edgenum++) {   
				int targ = destArray[edgenum];
				float weight = weightArray[edgenum];
				// REMOVAL from cluster k2: subtracts imbalance
				if(s_cluster[targ] == k2) {  // same cluster
					if(weight < 0) {
						negativeSumK1 -= fabs(weight);
					}
			...
			for(uint countK3 = 0; countK3 <= nc; countK3++) {  // cluster(k3)
				if(k3 != k1) {
					...
					for (ulong edgenum = offset_i; edgenum < count; edgenum++) {   
						int targ = destArray[edgenum];
						float weight = weightArray[edgenum];
						// ADDITION to cluster k3 != k1: adds imbalance
						if(s_cluster[targ] == k3) {  // same cluster
							if(weight < 0) {
								negativeSum += fabs(weight);
							}
						} else {  // diff cluster
					...
					for(uint k4 = k3 + 1; k4 <= nc; k4++) {  // cluster(k4)
						if(k4 != k2) {
							...
							ulong count2 = offset_j + numedges_j;
							// in/out-edges of vertex j
							for (ulong edgenum = offset_j; edgenum < count2; edgenum++) {   
								int targ = destArray[edgenum];
								float weight = weightArray[edgenum];
								// ADDITION to cluster k4 != k2: adds imbalance
								if(s_cluster[targ] == k4) {  // same cluster
									if(weight < 0) {
										negativeSum += fabs(weight);
									}
								} else {  // diff cluster

...
def main(argv):
   ...
   for root, subFolders, files in os.walk(folder):
	 ...
	 # Process CC results 
		   ...
		   try:
		    ...
		    for row in reader:
		       ...
		       if linestring.startswith( 'Best value' ):          
		          ...
		          time = float(column[6])
		          total_iter = line_count - 1  # long(column[7])
		 ...
		 if filename == previous_filename:
		    ...
		    avg_k = avg_k + best_K
		    avg_time.append(global_time)
		    avg_iter.append(total_iter)
		    avg_count = avg_count + 1
		 else:
		    if avg_count > 0:
		        print "storing " + previous_filename + ", number of executions = " + str(avg_count)
		        arr_iter = numpy.array(avg_iter)
		        arr_time = numpy.array(avg_time)
		        avg_file_summary[previous_filename] = str(avg_ip_const / avg_count) + ", " + str(avg_value / avg_count)+", "+str(avg_k / avg_count)+", "+str(numpy.mean(arr_time, axis=0))+", "+str(numpy.std(arr_time, axis=0))+", "+str(numpy.mean(arr_iter, axis=0))+", "+str(numpy.std(arr_iter, axis=0))+", "+str(avg_comb / avg_count)+", "+str(avg_count)
   ...
   print "Instance, Avg I(P) const, Avg I(P), Avg K, Avg Time(s), Stdev(Time), Avg Iter, Stdev(Iter), Avg combinations, Num executions"

...
Clustering NeighborhoodSearch::search1opt(SignedGraph* g,
	...
	const std::vector<unsigned long>& cluster = clustering->getVertexInClusterVector();
	const std::vector<unsigned long>& clusterIdList = clustering->getClusterIdList();
	...
	for (unsigned long i = uniN(), cont2 = 0; cont2 < n; cont2++, i = (i + 1) % n) {
		...
		unsigned long k1 = cluster[i];
		for (unsigned long c2 = uninc(), cont3 = 0; cont3 < numberOfClustersInInterval; cont3++) {
			unsigned long k2 = clusterIdList[c2];
			if (k1 != k2) {
				// cluster(k2)
				// removes node i from cluster k1 and inserts into cluster k2
...
Clustering NeighborhoodSearch::search2opt(SignedGraph* g,
	...
	for (unsigned long i = uniN(), conti = 0; conti < n; conti++, i = (i + 1) % n) {
		...
		for (unsigned long j = uniN(), contj = 0; contj < n; contj++, j = (j + 1) % n) {
			if(j == i)  continue;
			unsigned long k2 = cluster[j];
			// Option 1: node i is moved to another existing cluster k3
			for (unsigned long c3 = uninc(), contc3 = 0; contc3 < totalNumberOfClusters; contc3++) {
				MPI_IPROBE_RETURN(cBest)
				unsigned long k3 = clusterIdList[c3];
				if (k1 != k3) {
					// cluster(k3)
					// Option 1: node i is moved to another existing cluster k3 and
					//           node j is moved to another existing cluster k4
					for (unsigned long c4 = c3 + 1; c4 < totalNumberOfClusters; c4++) {
						unsigned long k4 = clusterIdList[c4];
						if (k2 != k4) {
							// cluster(k4)
							//BOOST_LOG_TRIVIAL(trace) << "Option 1: node " << i << " into cluster " << k3 << " and node " << j << " into cluster " << k4;
							cTemp = *clustering;
							NeighborhoodSearch::process2optCombination(*g, cTemp, problem, k1, k2, k3, k4, n, i, j);
							// cTemp->printClustering();
							Imbalance newImbalance = cTemp.getImbalance();
							if (newImbalance < bestImbalance) {
					...
					if((problem->getType() == ClusteringProblem::CC_PROBLEM) ||
						...
						cTemp = *clustering;
						NeighborhoodSearch::process2optCombination(*g, cTemp, problem, k1, k2, k3, Clustering::NEW_CLUSTER,	n, i, j);
						// cTemp->printClustering();
						Imbalance newImbalance = cTemp.getImbalance();
						if (newImbalance < bestImbalance) {
							//BOOST_LOG_TRIVIAL(trace) << "Better solution found in 2-neighborhood.";
			...
			if((problem->getType() == ClusteringProblem::CC_PROBLEM) ||
					( (problem->getType() == ClusteringProblem::RCC_PROBLEM)
							&& (clustering->getNumberOfClusters() < k) )) {
				for (unsigned long c4 = uninc(), contc4 = 0; contc4 < totalNumberOfClusters; contc4++) {
					unsigned long k4 = clusterIdList[c4];
					if (k2 != k4) {
						// cluster(k4)
						//BOOST_LOG_TRIVIAL(trace) << "Option 3: " << i << " into new cluster and " << j << " into cluster " << k4;
						cTemp = *clustering;
						NeighborhoodSearch::process2optCombination(*g, cTemp, problem, k1, k2, Clustering::NEW_CLUSTER,	k4, n, i, j);
						//cTemp->printClustering();
						Imbalance newImbalance = cTemp.getImbalance();
						if (newImbalance < bestImbalance) {
							//BOOST_LOG_TRIVIAL(trace) << "Better solution found in 2-neighborhood.";
			...
			if((problem->getType() == ClusteringProblem::CC_PROBLEM) ||
				...
				cTemp = *clustering;
				NeighborhoodSearch::process2optCombination(*g, cTemp, problem, k1, k2, Clustering::NEW_CLUSTER, Clustering::NEW_CLUSTER, n, i, j);
				// cTemp->printClustering();
				Imbalance newImbalance = cTemp.getImbalance();
				if (newImbalance < bestImbalance) {
					//BOOST_LOG_TRIVIAL(trace) << "Better solution found in 2-neighborhood.";
